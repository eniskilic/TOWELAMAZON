...
def generate_manufacturing_label(c, data, is_first=True):
    ...
    if data['has_gift_note']:
        c.setFont("Helvetica-Bold", 14)  # Enlarged from 11 to 14
        c.setFillColor(colors.HexColor('#D32F2F'))  # Red color
        c.drawString(left + c.stringWidth(data['buyer'], "Helvetica-Bold", 13) + 0.7 * inch, y, "GIFT")  # Moved 0.5" more to the right (was 0.2)
    ...
    if num_customizations >= 6:
        content_height = 2.8 * inch  # Set to 2.8 to keep borders on page
    else:
    ...
    if num_customizations >= 6:
        label_spacing = 0.15 * inch  # Good readability
        text_spacing = 0.26 * inch   # Adjusted to fit in 2.8" box
        label_font_size = 10
        text_font_size = 13
    else:
    ...
    for i, (label, text) in enumerate(data['customizations']):
        ...
        min_space_needed = 0.08 * inch if num_customizations >= 6 else 0.2 * inch
