    ...
    with pdfplumber.open(pdf_file) as pdf:
        ...
        for page in pdf.pages:
            ...
            if 'Order ID:' in text:
                ...
                if m:
                    current['order_date'] = m.group(1).strip()
                m = re.search(r'Shipping Service:\s*(.+?)(?:\n|Buyer)', text)
                if m:
                    current['shipping_service'] = m.group(1).strip()
                m = re.search(r'Ship To:\s*\n\s*(.+?)(?:\n)', text)
                if m:
                    current['buyer_name'] = m.group(1).strip()
...
def generate_manufacturing_label(c, data):
    ...
    if n >= 6:
        content_h = SIX_PC_CONTENT_IN * inch
    elif n >= 3:
        content_h = THREE_PC_CONTENT_IN * inch
    else:
        content_h = FEW_CONTENT_IN * inch
    ...
    c.setLineWidth(2)
    c.rect(left, content_bottom, right - left, content_h, stroke=1, fill=0)
    c.setLineWidth(1.5)
    c.line(left_right + 0.04 * inch, content_top, left_right + 0.04 * inch, content_bottom)
    ...
    c.setLineWidth(0.5)
    c.line(left + 0.05 * inch, col_y, left_right - 0.05 * inch, col_y)
    col_y -= 0.22 * inch

    c.setFont("Helvetica-BoldOblique" if int(data['quantity']) > 2 else "Helvetica-Bold", 18)
    c.drawCentredString(col_c, col_y, f"QTY: {data['quantity']}")
    col_y -= 0.22 * inch
...
def robust_merge_by_buyer_name(shipping_pdf_bytes, manufacturing_pdf_bytes, order_dataframe):
    try:
        ...
        with pdfplumber.open(shipping_pdf_bytes) as plist:
            shipping_pdf_bytes.seek(0)  # reset cursor after pdfplumber consumed the stream
...
if uploaded_files:
    ...
    if all_orders:
        ...
        with tab2:
            ...
            with c3:
                st.metric("Production Units", int(df_mfg['Mfg_Units'].sum()))
            with c4:
                st.metric("Gift Notes", int((df['Gift Message'] == 'YES').sum()))
